...
export default function SongPlayer({ track, accentClass = "", panelClass = "" }) {
  ...
  // Nothing configured yet — say so plainly instead of faking a player.
  if (!track.audio && !track.embed) {
    return (
      <div className={panel}>
        <TrackMeta index={track.index} title={track.title} artist={track.artist} />
        <p className="mt-3 text-xs opacity-45 font-sans">
          playback isn't configured for this track yet — see the README to add
          a file or a Spotify link.
        </p>
      </div>
    );
  }
